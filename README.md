...
A fully playable chess app: local two-player, vs-computer and online multiplayer
over WebSockets.
Framework-free rules engine, SolidJS UI with click and drag-and-drop input, strict
TypeScript, 136 tests.
...
- Local hot-seat play, vs-computer (Easy / Medium / Hard) and online rooms with
  shareable codes
...
- `src/ai.ts` — dependency-free negamax search (alpha-beta, depth 1–3) over the
  engine's move generator: material eval, mate scoring, capture-first ordering.
...
- `src/use-local-game.ts` / `src/use-computer-game.ts` / `src/use-online-game.ts`
  — the three drivers. Small focused hooks around them: `use-game-meta` (derivations),
...
`bun test` runs 136 tests: engine rules (including fool's and scholar's mate, pins,
promotion, castling rights), AI search (mate in 1, stalemate scoring, legal
openers on every level), rooms-reducer units, and full client↔server flows over
